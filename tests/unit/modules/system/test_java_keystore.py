...
from ansible_collections.ansible.community.tests.unit.modules.utils import ModuleTestCase, set_module_args
...
from ansible_collections.ansible.community.plugins.modules.java_keystore import create_jks, cert_changed, ArgumentSpec
...
class TestCreateJavaKeystore(ModuleTestCase):
    ...
    def setUp(self):
        ...
        self.mock_create_file = patch('ansible_collections.ansible.community.plugins.modules.java_keystore.create_file',
                                      side_effect=lambda path, content: path)
        self.mock_run_commands = patch('ansible_collections.ansible.community.plugins.modules.java_keystore.run_commands')
        ...
        self.mock_selinux_context = patch('ansible_collections.ansible.community.plugins.module_utils.basic.AnsibleModule.selinux_context',
                                          side_effect=lambda path: ['unconfined_u', 'object_r', 'user_home_t', 's0'])
        self.mock_is_special_selinux_path = patch('ansible_collections.ansible.community.plugins.module_utils.basic.AnsibleModule.is_special_selinux_path',
