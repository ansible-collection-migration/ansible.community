...
from ansible_collections.ansible.community.tests.unit.compat.mock import patch, MagicMock, PropertyMock
...
from ansible_collections.ansible.community.plugins.connection import ssh
...
class TestSSHConnectionRetries(object):
    ...
    def test_put_file_retries(self, monkeypatch):
        ...
        monkeypatch.setattr('time.sleep', lambda x: None)
        monkeypatch.setattr('ansible_collections.ansible.community.plugins.connection.ssh.os.path.exists', lambda x: True)
