...
from ansible_collections.ansible.community.tests.unit.compat import unittest, mock
from ansible_collections.ansible.community.plugins.module_utils.basic import AnsibleModule
from ansible_collections.ansible.community.plugins.module_utils.network.meraki.meraki import MerakiModule, meraki_argument_spec, HTTPError, RateLimitException
from ansible.module_utils.six import PY2, PY3
from ansible_collections.ansible.community.plugins.module_utils._text import to_native, to_bytes
...
def test_fetch_url_404(module, mocker):
    url = '404'
    mocker.patch('ansible_collections.ansible.community.plugins.module_utils.network.meraki.meraki.fetch_url', side_effect=mocked_fetch_url)
    mocker.patch('ansible_collections.ansible.community.plugins.module_utils.network.meraki.meraki.MerakiModule.fail_json', side_effect=mocked_fail_json)
...
def test_fetch_url_429_success(module, mocker):
    url = '429'
    mocker.patch('ansible_collections.ansible.community.plugins.module_utils.network.meraki.meraki.fetch_url', side_effect=mocked_fetch_url_rate_success)
    mocker.patch('ansible_collections.ansible.community.plugins.module_utils.network.meraki.meraki.MerakiModule.fail_json', side_effect=mocked_fail_json)
    mocker.patch('time.sleep', return_value=None)
