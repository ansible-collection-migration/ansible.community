...
from ansible_collections.ansible.community.plugins.module_utils.urls import (Request, open_url, urllib_request, HAS_SSLCONTEXT, cookiejar, RequestWithMethod,
                                       UnixHTTPHandler, UnixHTTPSConnection, httplib)
from ansible_collections.ansible.community.plugins.module_utils.urls import SSLValidationHandler, HTTPSClientAuthHandler, RedirectHandlerFactory
...
def urlopen_mock(mocker):
    return mocker.patch('ansible_collections.ansible.community.plugins.module_utils.urls.urllib_request.urlopen')


@pytest.fixture
def install_opener_mock(mocker):
    return mocker.patch('ansible_collections.ansible.community.plugins.module_utils.urls.urllib_request.install_opener')
...
def test_Request_open_ftp(urlopen_mock, install_opener_mock, mocker):
    mocker.patch('ansible_collections.ansible.community.plugins.module_utils.urls.ParseResultDottedDict.as_list', side_effect=AssertionError)
...
def test_Request_open_no_proxy(urlopen_mock, install_opener_mock, mocker):
    build_opener_mock = mocker.patch('ansible_collections.ansible.community.plugins.module_utils.urls.urllib_request.build_opener')
...
def test_methods(method, kwargs, mocker):
    ...
    open_mock = mocker.patch('ansible_collections.ansible.community.plugins.module_utils.urls.Request.open')
    request = Request()
...
def test_open_url(urlopen_mock, install_opener_mock, mocker):
    req_mock = mocker.patch('ansible_collections.ansible.community.plugins.module_utils.urls.Request.open')
